  ...
  while (stack.length > 0) {
    ...
    const nb = dirs
      .filter(([dx, dz]) => {
        const nx = cx + dx;
        const nz = cz + dz;
        return nx >= 0 && nx < w && nz >= 0 && nz < h && !grid[nz][nx].visited;
      })
...
function hits(boxes: WallBox[], px: number, pz: number): boolean {
  const r = P_RADIUS;
  for (const b of boxes) if (px + r > b.minX && px - r < b.maxX && pz + r > b.minZ && pz - r < b.maxZ) return true;
  return false;
}
...
export default function MazeEngine({ serverId, complexity = 5, equippedFlashlight, onPositionChange, onFlashlightChange }: MazeEngineProps) {
  ...
  useEffect(() => {
    ...
    const wallsH: THREE.Matrix4[] = [];
    const wallsV: THREE.Matrix4[] = [];
    ...
    for (let z = 0; z < mh; z += 3) for (let x = 0; x < mw; x += 3) {
      const pl = new THREE.PointLight(lightColor, 1.6, CELL * 3 * 1.4, 1.8);
    ...
    const animate = () => {
      ...
      if (move.x !== 0 || move.z !== 0) {
        const nx = pl.x + move.x;
        const nz = pl.z + move.z;
        if (!hits(wb, nx, nz)) {
          pl.x = nx;
          pl.z = nz;
        } else if (!hits(wb, nx, pl.z)) {
          pl.x = nx;
        } else if (!hits(wb, pl.x, nz)) {
          pl.z = nz;
        }
      }
      ...
      entities.forEach(({ mesh, ox, oz }) => {
        const dx = pl.x - mesh.position.x;
        const dz = pl.z - mesh.position.z;
