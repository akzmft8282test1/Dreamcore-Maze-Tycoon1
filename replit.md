...
- Pointer Lock FPS 조작 (WASD, YXZ Euler, 마우스 감도 조절)
- **2차원 시스템**: 분홍 출구 문 → 2차원 드림코어 백룸식 미로 진입
  - 분홍 현관문 (금색 열쇠 손잡이), X키로 상호작용 → 열쇠 회전 → 문 열림 애니메이션
  - 2차원: 8×8 백룸식 미로 (generateMaze 기반), 초록 잔디 바닥
  - 벽: 내부(복도)쪽 = 유리/반투명 + 분홍 테두리, 외부쪽 = 맑은 하늘+구름 텍스처
  - 비치볼: 물리 굴리기 (플레이어 접촉 시 밀림), X키로 수집, 엔티티 차단
  - 꽃눈 엔티티 (학교 교복 + 꽃 머리 + 큰 녹색 눈) — 3D 절차적 모델, 걷기 애니메이션
    - 눈(eyeMesh) Raycaster 감지: 2.25초 이상 응시 → 사망 → 1차원 복귀
    - 비치볼이 진로를 막으면 정지 (공 못 넘음)
  - 절벽: 미로 경계 밖으로 나가면 낙하 → 1차원 복귀
  - 인벤토리 HUD (수집 비치볼 수 표시), 응시 게이지 바, 낙하/사망 오버레이
  - GAZE_DEATH_TIME = 2.25초 (응시 사망 시간)
- `public/entity-flower.png`: 꽃눈 엔티티 원본 이미지 (3D 모델 참조용)
