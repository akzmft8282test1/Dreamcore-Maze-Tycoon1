...
- **2차원 시스템**: 분홍 출구 문 → 2차원 드림코어 세계 진입
  - 분홍 현관문 (금색 열쇠 손잡이), X키로 상호작용 → 열쇠 회전 → 문 열림 애니메이션
  - 2차원: 초록 잔디 바닥, 한쪽에서만 보이는 벽 패널, 비치볼, 절벽
  - 꽃눈 엔티티 (학교 교복 + 꽃 눈) 스프라이트 — 7.25초 이상 눈 마주침 → 사망 → 1차원 복귀
  - 사망 시 응시 게이지 바 + 사망 오버레이 표시
- `public/entity-eye.png`: 꽃눈 엔티티 이미지 (Two.js Sprite 빌보드)
